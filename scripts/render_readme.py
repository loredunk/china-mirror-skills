...
{% if quick_use.get(cat_id) %}
<details>
<summary>🚀 不安装 Skill 也能用：复制下面这段（推荐镜像：<b>{{ quick_use_recommended[cat_id].name }}</b>）</summary>
...
def status_badge(mirror: dict) -> str:
    ...
    # Deprecated/community status from mirrors.yml outranks live health: even
    # if a deprecated endpoint still responds, we don't want to label it 正常.
    declared = mirror.get('status')
...
def generate_readme(mirrors_data: dict, report: dict = None) -> str:
    ...
    for cat_id, cat_mirrors in mirrors_by_category.items():
        ...
        # Sort: active first, deprecated last; within each tier rank by live
        # latency (when we have a health report) then static priority.
        status_rank = {'active': 0, 'testing': 1, 'community': 1, 'deprecated': 2}
        cat_mirrors.sort(
            key=lambda mirror: (
                status_rank.get(mirror.get('status'), 1),
    ...
    # Render per-category quick-use snippets using the recommended mirror.
    # "Recommended" = first active mirror after live-rank/priority sort; if no
    # active mirror, fall back to the first non-deprecated one so the README
    # never shows quick-use for a deprecated mirror.
    quick_use_per_category: dict[str, str] = {}
    quick_use_recommended: dict[str, dict] = {}
    for cat_id, cat_info in categories.items():
        template_str = cat_info.get('quick_use_template')
        if not template_str:
            continue
        cat_mirrors = mirrors_by_category.get(cat_id, [])
        recommended = next(
            (m for m in cat_mirrors if m.get('status') == 'active'),
            None,
        )
        if recommended is None:
            continue
        url = recommended['url']
        sub_tpl = env.from_string(template_str)
        quick_use_per_category[cat_id] = sub_tpl.render(
            url=url,
            url_clean=url.rstrip('/'),
            host=urlparse(url).netloc,
            name=recommended['name'],
            homepage=recommended.get('homepage', url),
        )
        quick_use_recommended[cat_id] = recommended
    ...
    return template.render(
        ...
        quick_use=quick_use_per_category,
        quick_use_recommended=quick_use_recommended,
    )
